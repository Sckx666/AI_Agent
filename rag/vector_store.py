...
class VectorStoreService:
    ...
    def load_document(self):
        ...
        # 获取文件，根据不同后缀进行不同的读取方式
        def get_file_documents(read_path: str) -> list:
            if read_path.endswith("txt"):
                return txt_loader(file_path=read_path)

            if read_path.endswith("pdf"):
                return pdf_loader(file_path=read_path)

            return []

        # 把可以被处理的文件目录存入元组(不可被修改)
        ...
        for path in allowed_file_path:
            ...
            if not md5_hex:
                logger.warning(f"[MD5计算失败]{path}，跳过加载")
                continue
            ...
            try:
                ...
                # 记录这个已经处理过的md5,避免下次重复加载
                save_md5_hex(md5_hex)
                logger.info(f"[加载知识库]{path}内容加载成功")
            except Exception as e:
                # exc_info为True的话会记录详细的报错堆栈,如果为False仅记录报错信息本身
